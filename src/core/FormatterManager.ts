...
export default class FormatterManager {
	...
	formatDocument(
		...
		this.resetState();
		return this.formatLines(document, 0, document.lineCount - 1, options, token);
	}
	...
	formatRange(
		...
		return this.formatLines(document, range.start.line, range.end.line, options, token);
	}
	private formatLines(
		document: vscode.TextDocument,
		startLine: number,
		...
		for (let i = startLine; i <= endLine; i++) {
			...
			const indent =
				coreOptions.indent_char!.repeat(totalIndent * coreOptions.indent_size!) + this.state.rangeLeftSpace;
