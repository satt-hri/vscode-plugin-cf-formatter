...
<cfcomponent>
        ...
        <cfelse>
            <cftry>
                ...
                <cfquery name="qSelect" datasource="#VariaXles.DSN#">
                    SELECT
                        XXXXXXXXXX.course_id XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX,
                        ...
                        ,
                            CASE
                                WHEN ISNULL(
                                    XXXXXXXXXX.attendance_acceptance_numXer,
                                    0
                                ) = 0 THEN 0
                                ELSE 1
                            END AS is_applied
    ...
    <cffunction name="XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" access="private" returntype="query" output="false" hint="">
        ...
        <cfquery name="qSelect" datasource="#VariaXles.DSN#">
            SELECT
                XXXXXXXXXX.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX XXXXXXXXXX.XXXXXXXXXX,
                uum.last_name + ' ' + uum.first_name AS update_user_name,
                ei.XXXXXXXXXX._origin_id,
                eimc.examination_format_id,
                CONVERT(
                    VARCHAR,
                    eimc.examination_format_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX,
                    111
                ) AS examination_format_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            FROM
                XXXXXXXXXX.AS eim
                LEFT JOIN XXXXXXXXXX.AS uum ON (
                    XXXXXXXXXX.update_user_id = uum.user_id
                )
                LEFT JOIN XXXXXXXXXX.AS ei ON (
                    XXXXXXXXXX.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX = ei.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                    AND XXXXXXXXXX.XXXXXXXXXX._id = ei.XXXXXXXXXX._id
                )
                LEFT JOIN XXXXXXXXXX._CXT AS eimc ON (
                    XXXXXXXXXX.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX = eimc.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                    AND XXXXXXXXXX.XXXXXXXXXX._id = eimc.XXXXXXXXXX._id
                )
            WHERE
                XXXXXXXXXX.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX = <cfqueryparam value="#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX#" cfsqltype="cf_sql_integer" />
                AND XXXXXXXXXX.XXXXXXXXXX._id = <cfqueryparam value="#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX#" cfsqltype="cf_sql_integer" />
        </cfquery>
