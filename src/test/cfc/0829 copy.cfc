<cfcomponent>
	...
	<cffunction name="getCourseTreeCustom" access="remote" returntype="any" hint="コース構成を取得します。">
		...
		<cfquery name="qSelect" datasource="#Variables.DSN#">
			...
			FROM
				SCO_MASTER AS sm
				LEFT JOIN CONSTRUCT AS c ON (
					c.sco_id = sm.sco_id
					AND course_id = <cfqueryparam value="#courseID#" cfsqltype="cf_sql_integer" />
				)
			WHERE
				(1 = 1)
				<cfif courseID neq 0>
				...
				<cfif scoID neq 0>
					...
					AND sm.attendance_style_id = (
						SELECT
							sm2.attendance_style_id
						FROM
							sco_master AS sm2
						WHERE
							sm2.sco_id = <cfqueryparam value="#scoID#" cfsqltype="cf_sql_integer" />
					)
				</cfif>
			ORDER BY
				c.order_no,
		...
		<cfquery NAME="q_GetCmiInteractionsEntrycount" Datasource="#application.DSN#">
			...
			FROM
				ITEMS AS it
				INNER JOIN CONSTRUCT AS con ON (it.sco_id = con.sco_id)
			WHERE
				it.user_id = <cfqueryparam value="#Param1#" cfsqltype="cf_sql_integer" />
		...
		<cfquery NAME="q_SetlectureCount1" Datasource="#application.DSN#">
			...
			WHERE
				(user_id = <cfqueryparam value="#Param1#" cfsqltype="cf_sql_integer" />)
