    ...
    <cffunction name="get" access="private" returntype="any">
        ...
        <cftry>
            <cfscript>
                ...
                if(arguments.format eq "datatables" and arguments.course_id neq "all"){
                    throw(
                        errorcode = 400
                        ,type = "api_error"
                        ,message = "Invalid input"
				...
				cfloop(query=q_get_course){
					...
					if(arguments.with_sco){
						...
						var q_get_sco = queryNew("");
						{
							...
							";
							// 加工したSQLを実行
							q_get_sco = queryExecute(
								sql=sql
								,params={
									course_id={value=q_get_course.course_id, cfsqltype="CF_SQL_INTEGER"}
								}
								,options={
									datasource=Application.DSN
									,result="resultset"
